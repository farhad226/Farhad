...
import { Users, Calendar, TrendingUp, BarChart2, Loader2, ArrowUpRight, Trash2 } from 'lucide-react';
import { getVisitorStats, resetAnalytics } from '../../services/analyticsService';
...
export default function AnalyticsAdmin() {
  ...
  const handleReset = async () => {
    if (!window.confirm("Are you sure you want to reset all analytics data? This action cannot be undone.")) return;
    ...
    if (success) {
      ...
      alert("Failed to reset analytics. Check console for errors.");
    }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
        <div>
          <h2 className="text-2xl font-bold text-white">Visitor Overview</h2>
          <p className="text-gray-400 text-sm">Real-time tracking of your website traffic</p>
        </div>
        <button
          onClick={handleReset}
          disabled={isResetting}
          className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/30 rounded-lg transition-all text-sm font-medium disabled:opacity-50"
        >
